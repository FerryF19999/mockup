...
export function ProductQuickView({ name, price, note, badge, sprite, position, liveUrl = 'https://shop.nemu-ai.com/' }: Props) {
  ...
  return (
    <>
      <button className="mt-2 inline-flex items-center gap-1.5 text-[8px] font-black text-[#5b3fd5] transition hover:gap-2.5" type="button" onClick={() => setOpen(true)}>Lihat produk <ChevronRight size={12}/></button>
        ...
        <section className="relative w-full max-w-[880px] overflow-hidden rounded-[28px] bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label={`Lihat produk ${name}`}>
          ...
          <div className="grid md:grid-cols-[1.05fr_.95fr]">
            ...
            >
              ...
              <div className="absolute left-4 top-4 z-10 rounded-full border border-white/70 bg-white/85 px-3 py-2 text-[8px] font-black text-[#5b3fd5] shadow-sm backdrop-blur">TAMPILAN INTERAKTIF</div>
              <div
              ...
              <div className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center gap-1 rounded-full border border-white/70 bg-white/90 p-1.5 shadow-lg backdrop-blur">
                <button className="grid size-9 place-items-center rounded-full text-[#5b3fd5] transition hover:bg-[#f0edff] disabled:opacity-35" type="button" onClick={() => setSafeZoom(zoom - .25)} disabled={zoom <= 1} aria-label="Perkecil foto"><Minus size={16}/></button>
                <span className="min-w-11 text-center text-[8px] font-black text-[#554d60]">{Math.round(zoom * 100)}%</span>
                <button className="grid size-9 place-items-center rounded-full text-[#5b3fd5] transition hover:bg-[#f0edff] disabled:opacity-35" type="button" onClick={() => setSafeZoom(zoom + .25)} disabled={zoom >= 2.5} aria-label="Perbesar foto"><Plus size={16}/></button>
                <span className="mx-1 h-6 w-px bg-[#ddd6e7]"/>
                <button className="grid size-9 place-items-center rounded-full text-[#5b3fd5] transition hover:bg-[#f0edff]" type="button" onClick={() => setTilt((value) => Math.max(-18, value - 8))} aria-label="Putar tampilan ke kiri"><RotateCcw size={15}/></button>
                <button className="grid size-9 place-items-center rounded-full text-[#5b3fd5] transition hover:bg-[#f0edff]" type="button" onClick={() => setTilt((value) => Math.min(18, value + 8))} aria-label="Putar tampilan ke kanan"><RotateCw size={15}/></button>
                <button className="grid size-9 place-items-center rounded-full text-[#5b3fd5] transition hover:bg-[#f0edff]" type="button" onClick={resetView} aria-label="Kembalikan tampilan"><RefreshCw size={14}/></button>
              </div>
            </div>
            <div className="flex flex-col justify-center p-6 sm:p-9">
              ...
              <p className="mt-4 rounded-xl border border-[#e4dff0] bg-[#faf9fc] px-4 py-3 text-[8px] font-bold leading-5 text-[#716979]">Zoom buat cek detail. Saat diperbesar, geser fotonya. Saat normal, geser buat lihat sudut tampilannya.</p>
