...
export default function Home() {
  return (
    <main className="overflow-hidden bg-[#faf9f6] text-[#292333]">
      ...
      <section className="mx-auto max-w-[1240px] px-4 py-16 sm:px-6 lg:py-20">
        <div className="grid overflow-hidden rounded-[34px] border border-[#e4dfeb] bg-white shadow-[0_20px_70px_rgba(54,41,92,.10)] lg:grid-cols-[1.08fr_.92fr]">
          ...
          <div className="flex flex-col justify-center bg-[#453173] p-7 text-white sm:p-10 lg:p-12">
            <p className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[.14em] text-[#dfff5b]"><Store size={14}/> Mau ikut jualan?</p>
            <h2 className="mt-3 font-[var(--font-display)] text-4xl font-black leading-[.96] tracking-[-.055em] sm:text-5xl">Foto barangnya.<br/>Sisanya dibantu.</h2>
            <p className="mt-5 max-w-md text-sm leading-7 text-[#e5dcf2]">Nama, deskripsi, dan kategori bisa dirapikan NEMU. Harga dan stok tetap kamu yang pegang.</p>
            <div className="mt-6 flex flex-wrap gap-2">{['1 · Foto','2 · Cek hasilnya','3 · Mulai jualan'].map(item=><span className="rounded-full border border-white/20 bg-white/10 px-3 py-2 text-[8px] font-black" key={item}>{item}</span>)}</div>
      ...
      <section className="bg-[#f1eff4] px-4 py-12 sm:px-6 lg:py-16">
        <div className="mx-auto grid max-w-[1240px] gap-8 rounded-[30px] border border-[#e4dfeb] bg-white p-6 sm:p-8 lg:grid-cols-[.72fr_1.28fr] lg:p-10">
          <div className="lg:pr-8"><p className="text-[9px] font-black uppercase tracking-[.14em] text-[#5b3fd5]">Biar belanjanya tenang</p><h2 className="mt-3 text-3xl font-black tracking-[-.045em] sm:text-4xl">Yang penting kelihatan dari awal.</h2><p className="mt-4 max-w-md text-sm leading-7 text-[#6d6577]">Harga, kondisi, lokasi, dan info seller dibuat gampang dicek.</p></div>
          <div className="divide-y divide-[#e8e4f0]">{[[ShieldCheck,'Bayar lebih tenang','Pembayaran mengikuti alur pesanan.'],[BadgeCheck,'Seller gampang dicek','Profil, rating, lokasi, dan verifikasi kelihatan.'],[PackageCheck,'Kondisi nggak dibuat samar','Baru atau preloved ditulis dari awal.']].map(([Icon,title,copy])=>{const I=Icon as typeof ShieldCheck;return <div className="flex items-center gap-4 py-4 first:pt-0 last:pb-0" key={title as string}><span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#eff9d2] text-[#55720f]"><I size={19}/></span><div><h3 className="text-sm font-black">{title as string}</h3><p className="mt-1 text-[9px] leading-5 text-[#716979]">{copy as string}</p></div></div>})}</div>
        </div>
      </section>

      <section className="bg-white px-4 py-16 sm:px-6 lg:py-20" aria-labelledby="faq-heading">
        <div className="mx-auto max-w-[1100px]"><div className="mx-auto max-w-2xl text-center"><p className="text-[9px] font-black uppercase tracking-[.14em] text-[#5b3fd5]">Yang sering ditanyain</p><h2 id="faq-heading" className="mt-3 text-4xl font-black tracking-[-.05em] sm:text-5xl">Biar nggak kepikiran terus.</h2><p className="mt-4 text-sm text-[#6d6577]">Jawaban singkat buat hal yang paling sering bikin ragu.</p></div><div className="mt-9 grid gap-3 md:grid-cols-2">{faq.map(([question,answer])=><details className="faq-item group rounded-[22px] border border-[#e5e0ea] bg-[#faf9fb] px-5" key={question}><summary className="flex cursor-pointer list-none items-center justify-between gap-5 py-5 text-[11px] font-black"><span>{question}</span><CirclePlus className="shrink-0 text-[#5b3fd5]" size={19}/></summary><p className="pb-5 pr-6 text-[10px] leading-6 text-[#6d6577]">{answer}</p></details>)}</div></div>
      </section>

      <section className="bg-white px-4 pb-16 sm:px-6 lg:pb-20"><div className="mx-auto grid max-w-[1240px] items-center gap-6 rounded-[30px] bg-[#dfff5b] px-7 py-9 sm:px-10 lg:grid-cols-[1fr_auto]"><div><p className="text-[9px] font-black uppercase tracking-[.14em] text-[#526816]">Udah kebayang mau cari apa?</p><h2 className="mt-2 font-[var(--font-display)] text-3xl font-black tracking-[-.05em] text-[#30283a] sm:text-4xl">Bilang aja. Biar NEMU yang nyari.</h2></div><Link className="inline-flex w-fit items-center gap-3 rounded-full bg-[#5b3fd5] px-6 py-4 text-[9px] font-black text-white" href="/shop">Mulai belanja <ArrowRight size={16}/></Link></div></section>
      <Footer />
    </main>
  );
}
