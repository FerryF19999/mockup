    ...
    <main className="overflow-hidden bg-[#faf9f6] text-[#292333]">
      ...
      <section className="market-hero mx-auto max-w-[1240px] px-4 py-5 sm:px-6">
        <div className="grid min-h-[500px] overflow-hidden rounded-[28px] border border-[#e8e4f0] bg-white lg:grid-cols-[1.06fr_.94fr]">
          ...
          <div className="relative isolate flex min-h-[430px] items-center overflow-hidden bg-[#f1eeff] p-6 sm:p-8 lg:min-h-full">
            <div className="absolute -right-20 -top-20 -z-10 size-72 rounded-full bg-[#5b3fd5] opacity-10"/>
            <div className="w-full"><div className="mb-4 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-[#5b3fd5] p-4 text-white shadow-lg"><span className="grid size-8 place-items-center rounded-xl bg-white/15"><Search size={15}/></span><p className="mt-3 text-[8px] font-black uppercase tracking-[.11em] text-[#dfff5b]">Buat buyer</p><b className="mt-1 block text-[11px] leading-5">Cari pakai bahasa sehari-hari.</b></div><div className="rounded-2xl bg-[#dfff5b] p-4 text-[#34233d] shadow-lg"><span className="grid size-8 place-items-center rounded-xl bg-white/55"><Store size={15}/></span><p className="mt-3 text-[8px] font-black uppercase tracking-[.11em] text-[#526816]">Buat seller</p><b className="mt-1 block text-[11px] leading-5">Website, AI, dan data jualan.</b></div></div><div className="mb-3 flex items-center justify-between"><b className="text-[10px] text-[#292333]">Barang baru, preloved, dan lokal</b><Link className="flex items-center gap-1 text-[9px] font-black text-[#5b3fd5]" href="/shop#produk">Lihat semua <ChevronRight size={13}/></Link></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">{miniProducts.map((product,index)=><article className={`overflow-hidden rounded-2xl border border-white bg-white shadow-[0_10px_28px_rgba(54,41,92,.10)] ${index===2?'hidden sm:block lg:hidden xl:block':''}`} key={product.name}><div className="aspect-[1.08] w-full bg-cover bg-center bg-no-repeat" role="img" aria-label={product.name} style={{backgroundImage:`url('${product.sprite}')`,backgroundSize:'300% 200%',backgroundPosition:product.position}}/><div className="p-3 text-[#292333]"><h3 className="truncate text-[9px] font-extrabold">{product.name}</h3><strong className="mt-1 block text-xs text-[#5b3fd5]">{product.price}</strong><ProductQuickView {...product}/></div></article>)}</div><div className="mt-3 flex flex-wrap gap-2 text-[7px] font-black text-[#5d5469]">{['Gratis sampai pecah telur','AI foto + konten','Lihat channel yang cuan'].map(item=><span className="rounded-full border border-[#d9d2e5] bg-white/75 px-3 py-2" key={item}>{item}</span>)}</div></div>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1240px] px-4 py-16 sm:px-6 lg:py-20" id="kategori">
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-4 md:grid-rows-[190px_190px]">{categories.filter(({name})=>name!=='Elektronik').map(({name,position})=>{const featured=name==='Fashion';return <Link className={`group relative isolate overflow-hidden rounded-[22px] bg-[#ece8f4] transition duration-300 hover:-translate-y-1 hover:shadow-xl ${featured?'col-span-2 aspect-[1.55] md:row-span-2 md:aspect-auto':''}`} href="/shop" key={name}><span className="absolute inset-0 bg-cover bg-center bg-no-repeat transition duration-500 group-hover:scale-[1.035]" role="img" aria-label={`Kategori ${name}`} style={{backgroundImage:"url('/category-sprite-v1.png')",backgroundSize:'300% 200%',backgroundPosition:position}}/><span className="absolute inset-0 bg-gradient-to-t from-[#17151d]/80 via-transparent to-transparent"/><span className="absolute inset-x-0 bottom-0 flex items-end justify-between p-4 sm:p-5"><span><small className="inline-flex rounded-full bg-[#dfff5b] px-2.5 py-1 text-[7px] font-black uppercase tracking-[.12em] text-[#34233d] shadow-sm">Jelajahi</small><b className={`${featured?'text-2xl':'text-base'} mt-2 block text-white drop-shadow-md`}>{name}</b></span><span className="grid size-8 place-items-center rounded-full bg-white text-[#292333]"><ChevronRight size={15}/></span></span></Link>})}</div>
      </section>
