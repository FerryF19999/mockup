  ...
  return (
    <main className="overflow-hidden bg-[#FBFBFD] text-[#0B0B0E]">
      ...
      <section className="mx-auto mt-4 max-w-[1240px] px-4 sm:px-6" aria-labelledby="promo-heading">
        <div className="overflow-hidden rounded-[30px] border border-[#e1dce9] bg-[#0B0B0E] p-5 text-white shadow-[0_22px_60px_rgba(40,31,55,.12)] sm:p-7"><div className="flex items-end justify-between gap-4"><div><p className="text-[8px] font-black uppercase tracking-[.13em] text-[#EDE8FF]">Lagi ramai</p><h2 id="promo-heading" className="mt-1 text-2xl font-black tracking-[-.04em] sm:text-3xl">Banyak yang lagi ngincer ini</h2></div><a className="hidden items-center gap-1 text-[9px] font-black text-white sm:flex" href="#produk">Lihat semua <ChevronRight size={13}/></a></div><div className="mt-7 grid gap-3 md:grid-cols-6 md:grid-rows-[190px_190px]">{products.slice(0,5).map((product,index)=><article className={`group relative min-h-[210px] overflow-hidden rounded-[22px] bg-white text-[#0B0B0E] ${index===0?'md:col-span-2 md:row-span-2':'md:col-span-2'}`} key={product.name}><span className={`absolute bg-cover bg-center bg-no-repeat transition duration-500 group-hover:scale-105 ${index===0?'inset-0':'inset-y-0 left-0 w-[46%]'}`} role="img" aria-label={product.name} style={{backgroundImage:`url('${product.sprite}')`,backgroundSize:'300% 200%',backgroundPosition:product.position}}/>{index===0&&<span className="absolute inset-0 bg-gradient-to-t from-[#18131e]/95 via-[#18131e]/10 to-transparent"/>}<span className="absolute left-3 top-3 rounded-full bg-[#ff7d45] px-3 py-1.5 text-[7px] font-black text-white">{product.badge}</span><div className={`absolute ${index===0?'inset-x-0 bottom-0 p-5 text-white [&_button]:text-white':'inset-y-0 right-0 flex w-[54%] flex-col justify-center p-4'}`}><h3 className="text-[11px] font-black leading-4">{product.name}</h3><strong className={`mt-2 block text-base ${index===0?'text-[#EDE8FF]':'text-[#704BFD]'}`}>{product.price}</strong>{product.old&&<del className={`mt-0.5 block text-[7px] ${index===0?'text-white/60':'text-[#6B6B75]'}`}>{product.old}</del>}<div><ProductQuickView {...product}/></div></div></article>)}</div></div>
      </section>
      ...
      <section className="px-4 pb-16 sm:px-6"><div className="mx-auto max-w-[1240px] overflow-hidden rounded-[38px] bg-[#0B0B0E] text-white shadow-[0_30px_80px_rgba(34,25,47,.18)]"><div className="grid gap-0 lg:grid-cols-[1.08fr_.92fr]"><div className="p-7 sm:p-10 lg:p-12"><span className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[.14em] text-[#EDE8FF]"><ShieldCheck size={14}/> Biar belanjanya nggak waswas</span><h2 className="mt-3 text-4xl font-black leading-[.96] tracking-[-.055em] sm:text-5xl">Yang penting,<br/>kelihatan dari awal.</h2><div className="mt-8 grid gap-3 sm:grid-cols-2">{[[ShieldCheck,'Bayar lebih tenang','Alur pembayaran dan biaya kelihatan.'],[BadgeCheck,'Seller gampang dicek','Profil dan tanda verifikasinya jelas.'],[RotateCcw,'Ada masalah? Lapor.','Buka pesanan, lalu kirim laporan.'],[PackageCheck,'Kondisi nggak samar','Baru atau preloved ditulis dari awal.']].map(([Icon,title,copy])=>{const I=Icon as typeof ShieldCheck;return <div className="rounded-[20px] border border-white/12 bg-white/[.07] p-4" key={title as string}><I className="text-[#EDE8FF]" size={18}/><b className="mt-3 block text-[10px]">{title as string}</b><p className="mt-1 text-[8px] leading-4 text-white/60">{copy as string}</p></div>})}</div></div><div className="relative min-h-[460px] overflow-hidden"><span className="absolute inset-0 bg-cover bg-center" style={{backgroundImage:"url('/seller-packing-team-women-v1.jpg')"}}/><span className="absolute inset-0 bg-gradient-to-t from-[#211a29]/95 via-transparent to-transparent"/><div className="absolute inset-x-5 bottom-5 rounded-[24px] border border-white/15 bg-[#211a29]/80 p-5 backdrop-blur"><span className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[.14em] text-[#EDE8FF]"><Store size={13}/> Punya barang buat dijual?</span><h3 className="mt-2 text-2xl font-black tracking-[-.04em]">Foto. Kasih harga. Tayang.</h3><p className="mt-2 text-[9px] leading-5 text-white/65">NEMU bantu rapihin listing. Kamu tetap pegang harga dan tokomu.</p><a className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#EDE8FF] px-5 py-3 text-[9px] font-black text-[#0B0B0E]" href="https://seller.nemu-ai.com/register">Buka toko gratis <ArrowRight size={14}/></a></div></div></div></div></section>
      <Footer />
    </main>
  );
}
