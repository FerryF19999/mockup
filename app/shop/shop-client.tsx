...
export function AiShoppingAssistant() {
  ...
  return <div className="min-w-0">
    <form className="rounded-[24px] border border-[#d8d0ed] bg-[#FBFBFD] p-3 shadow-[0_14px_40px_rgba(74,52,135,.09)]" onSubmit={submit} role="search" aria-label="Mode AI NEMU">
      <div className="flex gap-3 px-2 pt-2"><span className="mt-1 grid size-10 shrink-0 place-items-center rounded-[14px] bg-[#704BFD] text-white shadow-md"><Bot size={19}/></span><textarea className="min-h-28 min-w-0 flex-1 resize-none bg-transparent py-2 text-sm font-semibold leading-6 text-[#0B0B0E] outline-none placeholder:font-medium placeholder:text-[#7B7482]" value={query} onChange={event=>setQuery(event.target.value)} placeholder="Contoh: Cariin sepatu putih buat kuliah, budget 300 ribuan, empuk, dan nggak gampang kotor." aria-label="Ceritakan barang yang kamu butuhkan"/></div>
