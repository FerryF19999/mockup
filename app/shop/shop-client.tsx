...
const suggestions = ['Baju kondangan di bawah Rp300 ribu', 'Kado buat ibu', 'HP bagus di bawah Rp1 juta'];

export function ShopSearch() {
  const [query, setQuery] = useState('');
  const [message, setMessage] = useState('');
  function submit(event: FormEvent) { event.preventDefault(); const value=query.trim(); setMessage(value ? `Oke, NEMU lagi nyariin “${value}”.` : 'Mau cari apa? Tulis dulu, ya.'); }
  return <div className="min-w-0"><form className="flex flex-col gap-2 rounded-2xl bg-white p-2 shadow-sm sm:flex-row sm:items-center" onSubmit={submit} role="search"><span className="hidden size-9 shrink-0 place-items-center rounded-xl bg-violet-50 text-violet-700 sm:grid"><Sparkles size={17}/></span><input className="min-w-0 flex-1 rounded-xl bg-zinc-50 px-4 py-3 text-sm text-zinc-900 outline-none placeholder:text-zinc-500 focus:ring-2 focus:ring-violet-200" value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Contoh: sepatu putih di bawah Rp200 ribu" aria-label="Cari produk dengan NEMU"/><Button className="nemu-astryx-button" label="Cariin" variant="primary" size="lg" type="submit" endContent={<ArrowRight size={15}/>} /></form><div className="mt-2 flex gap-1.5 overflow-x-auto [scrollbar-width:none]">{suggestions.map(item=><button className="shrink-0 rounded-full border border-violet-200 bg-white/70 px-3 py-1.5 text-[8px] font-semibold text-violet-800 transition hover:bg-white" type="button" key={item} onClick={()=>{setQuery(item);setMessage('')}}>{item}</button>)}</div><p className="mt-1.5 min-h-3 text-[8px] font-semibold text-violet-800" role="status" aria-live="polite">{message}</p></div>;
}
