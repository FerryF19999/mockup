...
export function splitNemuCosts(coreCharged: boolean, live: keyof typeof livePlans, term: 1 | 6 | 12, operations: number, marketing: ReturnType<typeof calculateMarketingBudget>): NemuCostSplit {
  return { coreCharged, live, term, addons: marketing.social + marketing.items.filter(item => item.id === 'nemu').reduce((sum, item) => sum + item.total, 0), sellerOperations: operations + marketing.items.filter(item => item.id !== 'nemu').reduce((sum, item) => sum + item.total, 0) };
}
