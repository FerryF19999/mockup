    ...
    <div className="sticky top-0 z-50 border-b border-zinc-200 bg-white/95 shadow-sm backdrop-blur-xl">
      <div className="bg-violet-950 text-white">
        <div className="mx-auto flex h-8 max-w-[1180px] items-center justify-between px-4 text-[10px] font-semibold sm:px-6">
          <span className="flex items-center gap-1.5"><Sparkles size={12} className="text-lime-300" /> Gratis ongkir untuk pesanan pertama</span>
      ...
      <header className="mx-auto grid min-h-[76px] max-w-[1180px] grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-3 sm:gap-7 sm:px-6">
        ...
        <form className="group flex h-11 min-w-0 items-center rounded-xl border border-zinc-200 bg-zinc-50 pl-3 transition focus-within:border-violet-400 focus-within:bg-white focus-within:ring-4 focus-within:ring-violet-100" action="/shop" role="search"><Search size={18} className="shrink-0 text-zinc-400 group-focus-within:text-violet-600" /><input className="min-w-0 flex-1 bg-transparent px-3 text-xs outline-none placeholder:text-zinc-400" name="q" placeholder="Cari produk, merek, atau tulis kebutuhanmu" aria-label="Cari produk" /><button className="m-1 hidden h-9 rounded-lg bg-violet-600 px-5 text-[10px] font-extrabold text-white transition hover:bg-violet-700 sm:block" type="submit">Cari</button></form>
  ...
  return (
    <footer className="bg-zinc-950 px-4 pb-7 pt-16 text-white sm:px-6">
      <div className="mx-auto grid max-w-[1180px] gap-10 border-b border-zinc-800 pb-14 md:grid-cols-[1.4fr_repeat(3,1fr)]">
        <div><Logo light /><p className="mt-6 max-w-xs text-sm leading-7 text-zinc-300">Mau cari barang atau mulai jualan? Di NEMU, semuanya dibikin lebih gampang.</p><div className="mt-6 flex gap-2"><a className="grid size-9 place-items-center rounded-lg border border-zinc-800 text-zinc-300 transition hover:border-violet-500 hover:text-white" href="https://www.instagram.com/nemu_ai_/" aria-label="Instagram"><Camera size={16} /></a><a className="grid size-9 place-items-center rounded-lg border border-zinc-800 text-zinc-300 transition hover:border-violet-500 hover:text-white" href="https://www.tiktok.com/@nemu_ai_" aria-label="TikTok"><Music2 size={16} /></a><a className="grid size-9 place-items-center rounded-lg border border-zinc-800 text-zinc-300 transition hover:border-violet-500 hover:text-white" href="mailto:hello@nemu-ai.com" aria-label="Email"><Mail size={16} /></a></div></div>
        {[
      ...
      <div className="mx-auto flex max-w-[1180px] flex-col gap-2 pt-6 text-[9px] text-zinc-600 sm:flex-row sm:justify-between"><span>© 2026 PT Nusa Era Modern Unggul</span><span>Dibuat untuk pembeli dan seller lokal Indonesia.</span></div>
    </footer>
  );
}
