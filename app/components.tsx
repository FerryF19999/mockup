...
export function Logo({ light = false }: { light?: boolean }) {
  return <Link className="inline-flex items-center" href="/" aria-label="NEMU AI beranda"><img className={`h-8 w-auto ${light?'brightness-0 invert':''}`} src="https://nemu-ai.com/_next/static/media/nemu-logo.d670f3d9.svg" alt="nemu.ai"/></Link>;
}
...
export function Header({ showSearch = true }: { showSearch?: boolean }) {
  ...
  const modeToggle = <div className={`mr-1 flex shrink-0 items-center rounded-full border p-1 transition duration-300 ${aiActive||switching?'border-[#704BFD] bg-[#17131F]':'border-[#ddd6e7] bg-white'}`} role="group" aria-label="Pilih cara mencari"><button className={`flex h-8 items-center gap-1.5 rounded-full px-2.5 text-[8px] font-black transition sm:px-3 ${!aiActive&&!switching?'bg-[#0B0B0E] text-white shadow':'text-[#6B6B75]'}`} type="button" onClick={()=>switchMode('/shop')} aria-pressed={!aiActive}><Search size={12}/><span className="hidden lg:inline">Cari biasa</span></button><button className={`flex h-8 items-center gap-1.5 rounded-full px-2.5 text-[8px] font-black transition sm:px-3 ${aiActive||switching?'bg-[#EDE8FF] text-[#0B0B0E] shadow':'text-[#704BFD] hover:bg-[#F2EFFF]'}`} type="button" onClick={()=>switchMode('/ai-mode')} aria-pressed={aiActive}><Sparkles size={12}/><span className="hidden lg:inline">Mode AI</span></button></div>;
  ...
  return (
    <div className="sticky top-0 z-50 bg-[#FBFBFD]/88 px-2 py-2 backdrop-blur-2xl sm:px-4">
      ...
      <header className="grid min-h-[68px] grid-cols-[auto_1fr_auto] items-center gap-3 px-3 py-2 sm:gap-6 sm:px-5">
        ...
        {showSearch ? <form className={`group flex h-12 min-w-0 items-center rounded-full border pl-4 transition duration-300 focus-within:ring-4 focus-within:ring-violet-100 ${switching?'scale-[1.015] border-[#704BFD] bg-[#ede8ff] shadow-xl':'border-[#cfc7dc] bg-[#f8f7fa] focus-within:border-[#704BFD] focus-within:bg-white'}`} action="/shop" role="search"><Search size={18} className="shrink-0 text-[#704BFD]" /><input className="min-w-0 flex-1 bg-transparent px-3 text-xs font-medium text-[#332d3d] outline-none placeholder:text-[#7c7487]" name="q" placeholder="Cari barang atau ceritain yang kamu mau" aria-label="Cari produk" />{modeToggle}</form> : <div className={`flex h-12 min-w-0 items-center justify-between rounded-full border px-2 pl-4 transition duration-300 ${aiActive?'border-[#704BFD] bg-[#17131F] text-white':'border-[#ded8e8] bg-[#f6f3fb] text-[#736a7d]'}`}><span className="flex min-w-0 items-center gap-2 text-[9px] font-bold"><Sparkles size={14} className="shrink-0 text-[#8a6df0]"/><span className="hidden truncate sm:block">{aiActive?'Ceritain aja, NEMU siap bantu':'Pilih cara kamu mau mencari'}</span></span>{modeToggle}</div>}
      ...
      <nav className="flex h-10 items-center gap-6 overflow-x-auto border-t border-[#eeeaf2] px-4 text-[9px] font-bold text-[#6B6B75] [scrollbar-width:none] sm:px-5" aria-label="Kategori populer"><Link className="flex shrink-0 items-center gap-2 font-black text-[#0B0B0E]" href="/shop"><Menu size={14} /> Semua kategori</Link>{['Lagi promo','Fashion','HP & gadget','Rumah','Beauty','Preloved'].map(item=><Link className="shrink-0 transition hover:text-[#704BFD]" href="/shop" key={item}>{item}</Link>)}</nav>
      </div>
    </div>
  );
}
