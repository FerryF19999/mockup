...
export function Logo({ light = false }: { light?: boolean }) {
  return <Link className="inline-flex items-center gap-2.5" href="/" aria-label="NEMU AI beranda"><span className={`grid size-10 place-items-center rounded-[14px] text-xl font-black ${light ? 'bg-white text-[#5b3fd5]' : 'bg-[#5b3fd5] text-[#dfff5b]'}`}>n</span><b className={`text-xl font-black tracking-[-0.06em] ${light ? 'text-white' : 'text-[#292333]'}`}>nemu.ai</b></Link>;
}
  ...
  return (
    <div className="sticky top-0 z-50 border-b border-[#e8e4f0] bg-white/95 backdrop-blur-xl">
      <div className="bg-[#5b3fd5] text-white">
        <div className="mx-auto flex h-9 max-w-[1240px] items-center justify-center px-4 text-[10px] font-bold sm:justify-between sm:px-6">
      ...
      <header className="mx-auto grid min-h-[78px] max-w-[1240px] grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-3 sm:gap-6 sm:px-6">
        <div className="[&>a>b]:hidden sm:[&>a>b]:block"><Logo /></div>
        <form className="group flex h-12 min-w-0 items-center rounded-full border-2 border-[#5b3fd5] bg-white pl-4 shadow-sm transition focus-within:ring-4 focus-within:ring-violet-100" action="/shop" role="search"><Search size={19} className="shrink-0 text-[#5b3fd5]" /><input className="min-w-0 flex-1 bg-transparent px-3 text-xs font-semibold text-[#332d3d] outline-none placeholder:font-medium placeholder:text-[#7c7487]" name="q" placeholder="Mau cari apa? Ketik aja..." aria-label="Cari produk" /><button className="m-1 hidden h-9 rounded-full bg-[#5b3fd5] px-6 text-[10px] font-black text-white transition hover:bg-[#4d32c4] sm:block" type="submit">Cari</button></form>
      ...
      <nav className="mx-auto flex h-11 max-w-[1240px] items-center gap-6 overflow-x-auto px-4 text-[10px] font-bold text-[#625b6d] [scrollbar-width:none] sm:px-6" aria-label="Kategori populer"><Link className="flex shrink-0 items-center gap-2 rounded-full bg-[#f0edff] px-3 py-2 font-black text-[#5b3fd5]" href="/shop"><Menu size={14} /> Semua kategori</Link>{['Lagi promo','Fashion','HP & gadget','Rumah','Beauty','Preloved'].map(item=><Link className="shrink-0 transition hover:text-[#5b3fd5]" href="/shop" key={item}>{item}</Link>)}<Link className="ml-auto flex shrink-0 items-center gap-1.5 font-black text-[#5b3fd5]" href="/shop#tanya-nemu"><Sparkles size={13} /> Bingung? Tanya NEMU</Link></nav>
    </div>
  );
}
