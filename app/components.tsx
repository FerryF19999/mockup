...
export function Header({ showSearch = true }: { showSearch?: boolean }) {
  ...
  return (
    <div className="sticky top-0 z-50 bg-[#FBFBFD]/88 px-2 py-2 backdrop-blur-2xl sm:px-4">
      ...
      <header className="grid min-h-[64px] grid-cols-[auto_1fr_auto] items-center gap-2 px-3 py-2 sm:min-h-[68px] sm:gap-6 sm:px-5">
      ...
      <nav className="flex h-10 snap-x items-center gap-5 overflow-x-auto border-t border-[#eeeaf2] px-4 text-[9px] font-bold text-[#6B6B75] [scrollbar-width:none] sm:gap-6 sm:px-5" aria-label="Kategori populer"><Link className="flex shrink-0 snap-start items-center gap-2 font-black text-[#0B0B0E]" href="/shop"><Menu size={14} /> Semua kategori</Link>{['Lagi promo','Fashion','HP & gadget','Rumah','Beauty','Preloved'].map(item=><Link className="shrink-0 snap-start transition hover:text-[#704BFD]" href="/shop" key={item}>{item}</Link>)}</nav>
      </div>
    </div>
  );
}
